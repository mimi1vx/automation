...
function custom_configuration()
{
    ...
    local loopnum=8
    for n in `seq 1 $loopnum` ; do
        echo "crowbar proposal edit: TRY #$n"
    ...
    if [[ $ret != 0 ]] ; then
        complain 88 "Error: 'crowbar $proposal proposal --file=$pfile edit $proposaltype' failed with exit code: $ret"
    fi
}
